import { Activity, BarChart3, Bell, BookOpenCheck, Database, Download, LineChart, LogOut, Menu, Search, Settings2, UserRound } from "lucide-react";

const NAV_ICONS = [BarChart3, Activity, LineChart, BookOpenCheck, Database];
const RAIL_LABELS = ["总览", "股票", "宏观", "图表", "报告"];

export function AppShell({ t, lang, stockQuery, activeNav, stockSourceStatus, macroSourceStatus, reportButtonRef, authUser, onOpenAuth, onLogout, onStockQueryChange, onLanguageChange, onNavigate, onOpenAiSettings, onExportReport, children }) {
  return <main className="terminal decision-terminal">
    ...
    <section className="workspace decision-workspace">
      <header className="topbar decision-command-bar">
        ...
        <div className="command-actions"><div className="segmented language-toggle" role="group" aria-label={t.language}><button type="button" className={lang === "zh" ? "selected" : ""} aria-pressed={lang === "zh"} onClick={() => onLanguageChange("zh")}>中</button><button type="button" className={lang === "en" ? "selected" : ""} aria-pressed={lang === "en"} onClick={() => onLanguageChange("en")}>EN</button></div>{authUser ? <><span className="command-account" title={authUser.email}><UserRound size={14} />{authUser.email}</span><button type="button" className="ghost command-auth" onClick={onLogout}><LogOut size={15} /> {t.account.logout}</button></> : <button type="button" className="ghost command-auth" onClick={onOpenAuth}><UserRound size={15} /> {t.account.login}</button>}<button type="button" className="ghost command-ai" data-ai-settings-trigger onClick={onOpenAiSettings}><Settings2 size={15} /> {t.ai.settings}</button><button type="button" className="ghost command-export" ref={reportButtonRef} onClick={onExportReport}><Download size={15} /> {t.export}</button></div>
      </header>
      {children}
