...
export function AiResearchPanel({ t, lang, ticker, score, status, result, error, needsPassword, onAnalyze, onRefresh, onExport, onSubmitPassword, onOpenSettings }) {
  ...
  return (
    <section className="panel ai-research-panel" aria-labelledby="ai-research-title" aria-busy={status === "loading" || exportState.status === "exporting"}>
      ...
      {!analysis && !needsPassword && <div className="ai-decision-summary"><div className="ai-score-card"><small>{t.ai.rating}</small><strong>{lang === "zh" ? "推荐" : "Recommended"}</strong><b>{score}<i>/100</i></b></div><div className="ai-score-card"><small>{t.ai.position}</small><strong>{lang === "zh" ? "中等偏多" : "Moderate long"}</strong><b>60<i>%</i></b></div><div className="ai-insight-block"><h3>{lang === "zh" ? "核心结论（简述）" : "Core thesis"}</h3><p>{lang === "zh" ? "基本面与流动性保持韧性，当前估值处于历史中枢附近，建议在风险可控范围内分批配置。" : "Fundamentals and liquidity remain resilient; consider disciplined accumulation within risk limits."}</p></div><div className="ai-insight-block opportunity"><h3>{lang === "zh" ? "机会点" : "Opportunities"}</h3><ul><li>{lang === "zh" ? "高端需求稳定，渠道库存健康" : "Resilient premium demand and inventory"}</li><li>{lang === "zh" ? "宏观流动性边际改善" : "Improving liquidity backdrop"}</li></ul></div><div className="ai-insight-block risk"><h3>{lang === "zh" ? "风险点" : "Risks"}</h3><ul><li>{lang === "zh" ? "需求恢复节奏不及预期" : "Demand recovery may lag"}</li><li>{lang === "zh" ? "估值波动加大" : "Valuation volatility"}</li></ul></div><button type="button" className="ai-run-button" onClick={onAnalyze} disabled={status === "loading"}>{status === "loading" ? <LoaderCircle className="spin" size={18} aria-hidden="true" /> : <Sparkles size={18} aria-hidden="true" />}{status === "loading" ? t.ai.analyzing : t.ai.analyze}</button></div>}
      {analysis && <div className="ai-analysis-content"><p className="sr-only" role="status" aria-live="polite">{t.ai.analysis}: {ticker}, {t.ai[analysis.rating]}</p><div className="ai-analysis-summary"><div><small>{t.ai.rating}</small><strong className={`ai-rating ${analysis.rating}`}>{t.ai[analysis.rating]}</strong></div><div><small>{t.ai.factorScore}</small><strong>{score}/100</strong></div><div><small>{t.ai.position}</small><strong>{analysis.position_range.min}%–{analysis.position_range.max}%</strong></div><p>{analysis.summary}</p></div>{visibleResult.research_evidence && <p className="ai-research-evidence">{lang === "zh" ? "研究证据版本" : "Research evidence version"} · {visibleResult.research_evidence.dataset_fingerprint.slice(0, 12)} · Sharpe {Number(visibleResult.research_evidence.metrics?.sharpe ?? 0).toFixed(2)}</p>}<div className="ai-analysis-grid"><article><h3>{t.ai.opportunities}</h3><ul>{analysis.opportunities.map((item) => <li key={item}>{item}</li>)}</ul></article><article><h3>{t.ai.risks}</h3><ul>{analysis.risks.map((item) => <li key={item}>{item}</li>)}</ul></article></div><article className="ai-watchlist"><h3>{t.ai.watchlist}</h3><div>{analysis.watchlist.map((item) => <span key={`${item.name}-${item.value}`}><b>{item.name}</b><em>{item.value}</em><small>{item.reason}</small></span>)}</div></article><div className="ai-analysis-meta"><span>{visibleResult.cached ? t.ai.cached : t.ai.generated} · {new Date(visibleResult.generated_at).toLocaleString()}</span><div className="ai-analysis-actions"><button type="button" className="ghost" onClick={handleExport} disabled={!canExport} aria-busy={exportState.status === "exporting"}>{exportState.status === "exporting" ? <LoaderCircle className="spin" size={14} /> : <Download size={14} />}{exportState.status === "exporting" ? t.ai.exportingPdf : t.ai.exportPdf}</button><button type="button" className="ghost" onClick={onRefresh} disabled={status === "loading" || exportState.status === "exporting"}><RefreshCcw className={status === "loading" ? "spin" : ""} size={14} /> {t.ai.refresh}</button></div></div>{exportState.error && <p className="ai-inline-error" role="alert">{t.ai.exportPdfFailed}</p>}</div>}
